...
Access and configure Jenkins using <Public-ip>:8080 on the web browser 
...
> [!NOTE]
> This pipeline is a multibranch one. This means, if there were more than one branch in GitHub, Jenkins would have scanned the repository to discover them all and we would have been able to trigger a build for each branch.
...
```
mysql -h (db private ip address) -u (username ) -p
```
...
for more information on [sonaqu](https://docs.sonarsource.com/sonarqube/latest/analyzing-source-code/scanners/jenkins-extension-sonarqube/)
